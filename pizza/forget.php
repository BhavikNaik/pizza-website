...
$mail = $_POST['mail']; 
        ...
        if(empty($username)){
            ...
            $sql = "SELECT * FROM user1 WHERE username='$name' AND mail='$mail'";

		    $result = mysqli_query($con, $sql);

            if (mysqli_num_rows($result) === 1) {
                $row = mysqli_fetch_assoc($result);
                if ($row['username'] === $name && $row['mail'] === $mail) {
...
<!--
      ...
      <div class="container register">
        <form action="forg.php" method="post" name="reg_form" onsubmit="return validate()">
        <div class="row">
